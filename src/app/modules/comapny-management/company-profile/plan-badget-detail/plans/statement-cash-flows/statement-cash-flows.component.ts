...
@Component({
  selector: 'PABudget-statement-cash-flows',
  templateUrl: './statement-cash-flows.component.html',
  styleUrls: ['./statement-cash-flows.component.scss'],
})
export class StatementCashFlowsComponent {
  @Input() inputData: any;
  ...
  getTreeTableData() {
    ...
    this.httpService
      .post<any>(StatementCashFlows.apiAddressStatementCashFlows, body)
      .pipe(
        map(response => {
  ...
  getTableData(comparison: number) {
    ...
    if (this.viewMode == 'table') {
      if (comparison == 1)
        url = StatementCashFlows.apiAddressStatementCashFlows;
